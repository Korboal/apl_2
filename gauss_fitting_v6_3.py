...
from lmfit import Model, Parameters, minimize, report_fit, Parameter
from lmfit.models import Gaussian2dModel, guess_from_peak2d
from scipy.interpolate import griddata
import datetime
from astropy.convolution import convolve, kernels, convolve_fft
from scipy.signal import convolve as sciconv
import matplotlib
#matplotlib.use('Agg')
default_fit_met_gauss2d = 'least_squares'

def gaussian2D(x, y, centerx, centery, sigmax, sigmay, amplitude, theta):  # TODO: find a way to optimise? Spending half of total time here
    """
    Model, also does convolution with global kernel parameter
    :param x: x values
    :param y: y values
    ...
    model = amplitude * np.exp(-((a * xdiff ** 2) + (b * xdiff * ydiff) + (c * ydiff ** 2)))

    #model = (amplitude * np.exp(-(((centerx - x) / sigmax) ** 2 + ((centery - y) / sigmay) ** 2) / 2.0))

    if np.size(model[0]) == np.size(kernel[0]):
        to_conv = kernel
    else:
        to_conv = kernel.flatten()
    values = sciconv(model, to_conv, mode='same')
    return values

def gauss2d_fitting_no_plot(data, x, y, error, psf_array, lya, fitting_method=default_fit_met_gauss2d, priors=np.zeros(16)):
    """
    :param image: takes image into it and converts into data already inside the function
    ...
    global kernel
    kernel_x_size = np.max(x) + 1  # 51
    kernel_y_size = np.max(y) + 1  # 51
    ...
    pars = get_param_guess(x, y, z, priors=priors)
    #out = mod.fit(z, params=pars, x=x, y=y, weights=1 / error, method=fitting_method)  #
...
def flatten_error(var):
    ...
    error = np.sqrt(output)
    error = np.nan_to_num(error, nan=-0.001)
    error = np.where(error < 0.0000001, np.mean(error), error)
    return error
...
def get_param_guess(x, y, z, priors=np.zeros(16)):
    ...
    pars = get_fit_pararmeters(*priors_to_send)
...
def get_fit_pararmeters(amp, min_amp, max_amp, centerx, min_cen_x, max_cen_x, centery, min_cen_y, max_cen_y, sigmax, min_sig_x, max_sig_x, sigmay, min_sig_y, max_sig_y, init_rot):
    pars = Parameters()
